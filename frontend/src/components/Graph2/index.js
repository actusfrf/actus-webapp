...
export class Graph2 extends React.Component {
  ...
  render() {
	...
	const gridLines = ticks.map(function(d) {
		let raw=[{time:xScale.domain()[0],value:d},
			{time:xScale.domain()[1],value:d}];
	...
	const eventLines = data.map(function(d) {
		let raw=[{time:d.time,payoff:0},
			{time:d.time,payoff:Math.abs(d.payoff)}];
	  return(<path
	...
	{/* draw axes*/}
 	<g className="xAxis" 
		ref={node => d3Select(node).call(xAxis)}
		 style={{ transform: `translateY(${height-2*margin}px)`, }} />
	<g className="yAxis" 
